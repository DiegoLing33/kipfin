    ...
    <div class="found-results">
      ...
      >
        ...
        <div class="f-cell">
          ...
          <TaggedComponent
              :tags="(user.tags || '').split(',').filter(v => v!=='')"/>
        </div>
      </div>
    </div>
...
@Component({
  components: {TaggedComponent, CollapseCard, UserAvatarBox, UserContent, RadioField, SelectField, TextField}
})
export default class AdminList extends Vue {
  ...
  private find() {
    ...
    if (this.searchTags) {
      this.result = items.filter(v => {
        const raw = (v.tags || '').toLocaleLowerCase();
