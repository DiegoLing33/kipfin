        ...
        <b-container v-else>
            ...
            <b-row class="rw align-items-center" v-for="group of groupsList" :key="group.studentGroupId">
                ...
                <b-col class="p-3">
                    ...
                    <router-link class="ml-3" target='_blank' :to="('/new/index.php?class=res&method=group&groupId=' +
                     group.studentGroupId  + '&token=' + $store.getters.accountAuthToken)">
                        <b-button
                                v-b-tooltip.hover title="Список на печать (для всех)"
                        >
                            <b-icon-list/>
                        </b-button>
                    </router-link>
                    <router-link class="ml-3" target='_blank' :to="('/new/index.php?class=res&method=group&p=1&groupId='
                    + group.studentGroupId + '&token=' + $store.getters.accountAuthToken)">
                        <b-button
                                v-b-tooltip.hover title="Список на печать (приватный)"
                        >
                            <b-icon-shield-lock/>
                        </b-button>
                    </router-link>
