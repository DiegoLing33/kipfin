 ...
 * @param src
 */
export function dict<T = never>(src: Dict<T> = {}): Dict<T> {
    return src || {};
}

export const Dict = {
    values<T = never>(a: Dict<T>): T[] {
        return Object.values(a);
    },
    keys<T = never>(a: Dict<T>): string[] {
        return Object.keys(a);
    },
    forEach<T = never>(a: Dict<T>, callback: (key: string, value: T) => void): void {
        Dict.keys(a).forEach(key => callback(key, a[key]));
    }
};
